require 'rails_helper'

# This spec was generated by rspec-rails when you ran the scaffold generator.
# It demonstrates how one might use RSpec to specify the controller code that
# was generated by Rails when you ran the scaffold generator.
#
# It assumes that the implementation code is generated by the rails scaffold
# generator.  If you are using any extension libraries to generate different
# controller code, this generated spec may or may not pass.
#
# It only uses APIs available in rails and/or rspec-rails.  There are a number
# of tools you can use to make these specs even more expressive, but we're
# sticking to rails and rspec-rails APIs to keep things simple and stable.
#
# Compared to earlier versions of this generator, there is very limited use of
# stubs and message expectations in this spec.  Stubs are only used when there
# is no simpler way to get a handle on the object needed for the example.
# Message expectations are only used when there is no simpler way to specify
# that an instance is receiving a specific message.

RSpec.describe BenefitsController, type: :controller do

  # This should return the minimal set of attributes required to create a valid
  # Benefit. As you add validations to Benefit, be sure to
  # adjust the attributes here as well.
  let(:valid_attributes) {
    {
      title: "Titulo",
      description: "Descricao"
    }
  }

  let(:invalid_attributes) {
    {
      title: "Titulo",
      description: nil
    }
  }

  # This should return the minimal set of values that should be in the session
  # in order to pass any filters (e.g. authentication) defined in
  # BenefitsController. Be sure to keep this updated too.

  describe "GET #index" do

    login_admin

    it "assigns all benefits as @benefits" do
      benefit = Benefit.create! valid_attributes
      get :index, {}
      expect(assigns(:benefits)).to eq([benefit])
    end
  end

  describe "GET #show" do

    login_admin
    it "assigns the requested benefit as @benefit" do
      benefit = Benefit.create! valid_attributes
      get :show, {:id => benefit.to_param}
      expect(assigns(:benefit)).to eq(benefit)
    end
  end

  describe "GET #new" do

    login_admin
    it "assigns a new benefit as @benefit" do
      get :new, {}
      expect(assigns(:benefit)).to be_a_new(Benefit)
    end
  end

  describe "GET #edit" do

    login_admin
    it "assigns the requested benefit as @benefit" do
      benefit = Benefit.create! valid_attributes
      get :edit, {:id => benefit.to_param}
      expect(assigns(:benefit)).to eq(benefit)
    end
  end

  describe "POST #create" do

    login_admin
    context "with valid params" do
      it "creates a new Benefit" do
        expect {
          post :create, {:benefit => valid_attributes}
        }.to change(Benefit, :count).by(1)
      end

      it "assigns a newly created benefit as @benefit" do
        post :create, {:benefit => valid_attributes}
        expect(assigns(:benefit)).to be_a(Benefit)
        expect(assigns(:benefit)).to be_persisted
      end

      it "redirects to the created benefit" do
        post :create, {:benefit => valid_attributes}
        expect(response).to redirect_to(Benefit.last)
      end
    end

    context "with invalid params" do
      it "assigns a newly created but unsaved benefit as @benefit" do
        post :create, {:benefit => invalid_attributes}
        expect(assigns(:benefit)).to be_a_new(Benefit)
      end

      it "re-renders the 'new' template" do
        post :create, {:benefit => invalid_attributes}
        expect(response).to render_template("new")
      end
    end
  end

  describe "PUT #update" do

    login_admin
    context "with valid params" do
      let(:new_attributes) {
        {
          title: "Titulo 2"
        }
      }

      it "updates the requested benefit" do
        benefit = Benefit.create! valid_attributes
        put :update, {:id => benefit.to_param, :benefit => new_attributes}
        benefit.reload
        expect(benefit.title).to eq(new_attributes[:title])
      end

      it "assigns the requested benefit as @benefit" do
        benefit = Benefit.create! valid_attributes
        put :update, {:id => benefit.to_param, :benefit => valid_attributes}
        expect(assigns(:benefit)).to eq(benefit)
      end

      it "redirects to the benefit" do
        benefit = Benefit.create! valid_attributes
        put :update, {:id => benefit.to_param, :benefit => valid_attributes}
        expect(response).to redirect_to(benefit)
      end
    end

    context "with invalid params" do
      it "assigns the benefit as @benefit" do
        benefit = Benefit.create! valid_attributes
        put :update, {:id => benefit.to_param, :benefit => invalid_attributes}
        expect(assigns(:benefit)).to eq(benefit)
      end

      it "re-renders the 'edit' template" do
        benefit = Benefit.create! valid_attributes
        put :update, {:id => benefit.to_param, :benefit => invalid_attributes}
        expect(response).to render_template("edit")
      end
    end
  end

  describe "DELETE #destroy" do

    login_admin
    it "destroys the requested benefit" do
      benefit = Benefit.create! valid_attributes
      expect {
        delete :destroy, {:id => benefit.to_param}
      }.to change(Benefit, :count).by(-1)
    end

    it "redirects to the benefits list" do
      benefit = Benefit.create! valid_attributes
      delete :destroy, {:id => benefit.to_param}
      expect(response).to redirect_to(benefits_url)
    end
  end

end
